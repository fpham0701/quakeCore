...
namespace quakecore {
                ...
                for (int cam = tid; cam < num_cameras; cam += stride) {
                    ...
                    while (sp > 0) {
                        ...
                        for (int c = 0; c < 2; ++c) {
                            ...
                            if (child >= 0) {
                                // bounds check happens on pop, this is only for safety not correctness
                                if (sp < kStackMax) {
                                    stack[sp++] = child;
                                }
                            } else {
    ...
    TraversalStats RunGpuOptimizedTraversal(const BspData& bsp, const std::vector<Camera>& cameras,
        ...
        // cudaMallocHost not used here because we do not get the added throughput benefit
        // we are paying for at typical BSP sizes (<= ~50MB transferred)
        ...
        // legacy stream used here b/c standard stream creation + cudaMallocAsync pool init costs
        // are not worth it for a single-shot kernel launch
        ...
        // explicitly avoiding oversubscription by matching threads executing to actual workload;
        // subscribing more than this would end up paying scheduling cost while leaving most threads
        // idle
        int sm_count = 1;
